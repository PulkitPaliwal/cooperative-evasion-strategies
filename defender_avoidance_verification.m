...
for index1 = 1:500
    FOUND = false;
    velocity_defender = index1/100;
    logger.weight(index1) = velocity_defender;
    for velocity = 0:0.01:4
        if FOUND
            break;
        end
        ...
        pursuer1.V   =  velocity; % velocity magnitude, m/s
        ...
        defender.V   =  velocity_defender; % velocity magnitude, m/s
        ...
        for k = 1:Niter
            ...
            if abs(R1) <= 0.25 || abs(R2) <= 0.25 || abs(R12) <= 0.25 || abs(RD1) <= 0.25
                if abs(R1) <= 0.25
                    disp("success")
                    disp(velocity_defender)
                    min_vel = velocity;
                    logger.min_vel(index1) = min_vel; 
                    disp(min_vel)
                    FOUND = true;
                
                end
                break;
            elseif k >Niter-1 && velocity> 3.8
                disp("criticial failure")
                break;
