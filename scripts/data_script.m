% RESULT CALCULATION SCRIPT
logger.weight   = nan(1, 1000);
logger.min_vel  = nan(1,1000);
weight = 0.55;
for index = 1:50
    found_min_vel = false;
    target_velocity_uni_direc = index/10.0;
    logger.weight(index) = target_velocity_uni_direc*1.414;
    for velocity = 0:0.1:1000
        if found_min_vel
            break;
        end
        ...
        defender.V   =  velocity; % velocity magnitude, m/s
        ...
        target.Nv  =  target_velocity_uni_direc; % north velocity, m/s
        target.Ev  =  target_velocity_uni_direc; % east velocity, m/s
        ...
        for k = 1:Niter
            ...
            if abs(R1) <= 0.25 || abs(R2) <= 0.25 || abs(R12) <= 0.25 || abs(RD1) <= 0.25 || abs(RD2) <= 0.25
                if abs(R12)<=0.25 && abs(R1) > 0.25 && abs(R2) > 0.25
                    found_min_vel = true;
                    disp("success");
                    disp("weight and min vel");
                    disp(index/100);
                    disp(velocity);
                    logger.min_vel(index) = velocity;
                end
                break;
            end
...
set(gca, 'DataAspectRatio',[300 100 100]);
title(['Pure Proportional Navigation, N = ' num2str(N) ])
legend('minimum defender velocity', 'weight')
xlabel('weight')
ylabel('minimum defender velocity')
